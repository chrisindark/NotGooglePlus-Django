...
def file_path_fn(instance, filename):
    ...
    file_path = settings.FILE_UPLOAD_PATH + f"{instance.file_name}/{instance.file_name}"
    return file_path
...
def file_directory_path_fn(instance):
    ...
    file_directory_path = (
        settings.MEDIA_PATH + settings.FILE_UPLOAD_PATH + f"{instance.file_name}"
    )
    return file_directory_path


def thumbnail_file_directory_fn(instance):
    thumbnail_file_directory_path = (
        settings.MEDIA_PATH + settings.FILE_THUMBNAIL_PATH + f"{instance.file_name}"
    )
...
class FileUpload(TimestampedModel):
    ...
    file = models.FileField(upload_to=file_path_fn, max_length=255)
