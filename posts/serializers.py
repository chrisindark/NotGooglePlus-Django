...
from posts.models import Post, Comment, FileUpload
import os
...
class FileUploadSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if len(data['file'].name) > 75:
            raise serializers.ValidationError({'file': 'File name should be less than or equal to 75 characters.'})
        ...
        if data['file_type'] == 'image' and data['file'].content_type not in ALLOWED_IMAGE_TYPES:
            raise serializers.ValidationError({'file': 'Image format should be of {0}.'.format(', '.join(ALLOWED_IMAGE_TYPES))})
        elif data['file_type'] == 'audio' and data['file'].content_type not in ALLOWED_AUDIO_TYPES:
            raise serializers.ValidationError({'file': 'Audio format should be of {0}.'.format(', '.join(ALLOWED_AUDIO_TYPES))})
        elif data['file_type'] == 'video' and data['file'].content_type not in ALLOWED_VIDEO_TYPES:
            raise serializers.ValidationError({'file': 'Video format should be of {0}.'.format(', '.join(ALLOWED_VIDEO_TYPES))})
